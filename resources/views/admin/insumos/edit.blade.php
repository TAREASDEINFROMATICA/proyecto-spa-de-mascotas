...
<html>
<head>
    <title>Editar Insumo - {{ $insumo->nombre }}</title>
...
<body>
        ...
        <div class="content">
            ...
            <div class="stock-card">
                <div>
                    ...
                    <div class="value">{{ $insumo->stock }} {{ $insumo->unidad_medida }}</div>
                </div>
                <div>
            ...
            <form method="POST" action="{{ route('admin.insumos.update', ['id' => $insumo->id_insumo, 'token' => $token]) }}" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                
                <div class="form-group">
                    <label><i class="fas fa-tag"></i> Nombre *</label>
                    <input type="text" name="nombre" value="{{ $insumo->nombre }}" required placeholder="Ej: Shampoo Hipoalergénico">
                </div>
                
                <div class="form-group">
                    <label><i class="fas fa-align-left"></i> Descripción</label>
                    <textarea name="descripcion" rows="3" placeholder="Descripción del insumo...">{{ $insumo->descripcion }}</textarea>
                </div>
                
                <div class="form-group">
                    <label><i class="fas fa-exclamation-triangle"></i> Stock Mínimo *</label>
                    <input type="number" name="stock_minimo" step="0.01" value="{{ $insumo->stock_minimo }}" required>
                ...
                <div class="form-group">
                    ...
                    <select name="unidad_medida" required>
                        ...
                        <option value="g" {{ $insumo->unidad_medida == 'g' ? 'selected' : '' }}>Gramos (g)</option>
                        ...
                        <option value="par" {{ $insumo->unidad_medida == 'par' ? 'selected' : '' }}>Par</option>
                    </select>
                </div>
                
                <div class="form-group">
                    <label><i class="fas fa-dollar-sign"></i> Costo Unitario (Bs)</label>
                    <input type="number" name="costo_unitario" step="0.01" value="{{ $insumo->costo_unitario }}" placeholder="0.00">
                </div>
                ...
                <div class="form-group">
                    ...
                    <div class="imagen-actual">
                        @if($insumo->imagen)
                            <img src="{{ Storage::url($insumo->imagen) }}" alt="{{ $insumo->nombre }}">
                            <p style="margin-top: 10px; font-size: 12px; color: #666;">
                                <i class="fas fa-check-circle" style="color: #4CAF50;"></i> Imagen actual del insumo
                            </p>
                        @else
                ...
                <div class="button-group">
                    ...
                    <a href="{{ route('admin.insumos.index', ['token' => $token]) }}" class="btn btn-secondary">
            ...
            <div class="alert alert-info">
                ...
                <div>Aquí puedes ajustar el stock manualmente para entradas (compras) o salidas (mermas, devoluciones).</div>
            </div>
            ...
            <div class="ajuste-stock">
                <div class="form-group">
                    ...
                    <select id="tipo_movimiento" required>
                        <option value="entrada">
                            <i class="fas fa-plus-circle"></i> ➕ Entrada (agregar stock)
                        </option>
                        <option value="salida">
                            <i class="fas fa-minus-circle"></i> ➖ Salida (retirar stock)
                        </option>
                    </select>
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="number" id="cantidad" step="0.01" min="0.01" placeholder="Ej: 10" required>
                </div>
                
                <div class="form-group">
                    <label><i class="fas fa-comment"></i> Motivo</label>
                    <input type="text" id="motivo" placeholder="Ej: Compra, Devolución, Merma, etc.">
                </div>
    ...
    <script>
        ...
        function ajustarStock() {
            ...
            fetch('/admin/insumos/' + insumoId + '/ajustar-stock?token=' + token, {
                method: 'POST',
                headers: {
